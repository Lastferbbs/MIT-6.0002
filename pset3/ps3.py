...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        tiles_dict: empty dict
        tiles_cleaned: an integer = 0
        """
        self.width = width
        self.height = height
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if (x, y) in self.tiles_dict:
            if self.tiles_dict[(x, y)] > 0:
                self.tiles_dict[(x, y)] = (
                    self.tiles_dict[(x, y)] - capacity
                    if self.tiles_dict[(x, y)] - capacity > 0
                    else 0
                )
                if not self.tiles_dict[(x, y)]:
                    self.tiles_cleaned += 1

        else:
            self.tiles_dict[(x, y)] = (
                self.dirt_amount - capacity if self.dirt_amount - capacity > 0 else 0
            )
            self.tiles_cleaned += not self.tiles_dict[(x, y)]
    ...
    def is_tile_cleaned(self, m, n):
        ...
        Note: The tile is considered clean only when the amount of dirt on this
              tile is 0.
        """
        if (m, n) in self.tiles_dict:
            return not self.tiles_dict[m, n]
        return False
        # raise NotImplementedError
    ...
    def is_position_in_room(self, pos):
        ...
        """
        if (math.floor(pos.get_x()) < self.width and math.floor(pos.get_x()) >= 0) and (
            math.floor(pos.get_y()) < self.height and math.floor(pos.get_y()) >= 0
        ):
            return True
        return False
...
class Robot(object):
    ...
    def get_robot_position(self):
        ...
        """
        # return Position(
        #     random.uniform(0.00, self.room.width),
        #     random.uniform(0.00, self.room.height),
        # )
    ...
    def update_position_and_clean(self):
        ...
        """
        # do not change -- implement in subclasses
        new_possible_position = self.get_robot_position().get_new_position(
            self.direction, self.speed
        )
        if self.room.is_position_valid(new_possible_position):
            self.set_robot_position(new_possible_position)
            self.room.clean_tile_at_position(new_possible_position, self.capacity)
        else:
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        return Position(
            random.uniform(0.00, self.width),
            random.uniform(0.00, self.height),
        )
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.gets_faulty():
            ...
            super().update_position_and_clean()
...
def run_simulation(
    ...
    for i in range(num_trials):
        ...
        time_steps_for_trials.append(time_steps)
    return sum(time_steps_for_trials) / len(time_steps_for_trials)
...
print(
    "avg time steps: " + str(run_simulation(1, 1.0, 1, 20, 20, 3, 0.5, 50, FaultyRobot))
)
print(
    "avg time steps: "
